      ...
      {items.map((it, i) => (
        <div key={it.label} className="flex items-center gap-1.5 sm:gap-2">
          <svg
            ...
            className="text-gold-600 md:h-[13px] md:w-[13px]"
          >
            <path d="M5 13l4 4L19 7" />
          </svg>
          <span className="text-[10px] uppercase tracking-[0.15em] text-charcoal-600 sm:tracking-[0.2em] md:text-[11px]">
            {it.label}
          </span>
          {i < items.length - 1 && (
            <span className="ml-3 hidden h-1 w-1 rounded-full bg-gold-500/40 md:ml-4 md:inline-block" />
          )}
        </div>
      ))}
    </div>
